...
$output_dir="output/$project/projector";
...
for($g=1; $g<=$numSimulations; $g+=1) {
    for($numNodes=128; $numNodes<=1024; $numNodes*=2) {
        ...
        "-project $project " .                                                    
        "-overwrite input=$input_file/$numNodes/$g\_tor_$numNodes.txt output=$output_dir/$numNodes/ " .   
        "AutoStart=true\n";

        system("java -cp binaries/bin:binaries/jdom.jar sinalgo.Run -batch " .
        "-project $project " .                                                           # choose the project
        "-overwrite input=$input_file/$numNodes/$g\_tor_$numNodes.txt output=$output_dir/$numNodes/ " .  # Overwrite configuration file parameters
